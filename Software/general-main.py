...
def main():
    ...
    breast_cancer_const = gen_rand_const(breast_cancer_set, breast_cancer_labels, breast_cancer_const, int(len(breast_cancer_labels) * const_percent), 0, 1)
    ...
    print("CECM")
    iris_cecm_assignment = CEKM(iris_set, 3, iris_const, alpha=1, rho=1000, bal=0)
    wine_cecm_assignment = CEKM(wine_set, 3, wine_const, 300, rho=1000, bal=0, stop_thr=0.5, alpha=1)
    breast_cancer_cecm_assignment = CEKM(breast_cancer_set, 2, breast_cancer_const, 300, rho=1000, bal=0, stop_thr=0.1, alpha=1)
    glass_cecm_assignment = CEKM(glass_set, 2, glass_const, 300, rho=1000, bal=0, stop_thr=0.5, alpha=1)
    digits_cecm_assignment = CEKM(digits_set, 10, digits_const, 300, rho=1000, bal=0, stop_thr=0.5, alpha=1)

    rand_cecm_assignment = CEKM(rand_set, 3, rand_const, alpha=1, rho=1000, bal=0)
    spiral_cecm_assignment = CEKM(spiral_set, 2, spiral_const, alpha=1, rho=1000, bal=0)
    moons_cecm_assignment = CEKM(moons_set, 2, moons_const, alpha=1, rho=1000, bal=0)
    circles_cecm_assignment = CEKM(circles_set, 2, circles_const, alpha=1, rho=1000, bal=0)
    ...
    # lamb_arr[::-1].sort()
    # iris_rdpm_assignment = RDPM(iris_set, 2, iris_const, 20000, 0.1, 1)[0]
    # T = np.mean(wine_set, 0)
    # lamb_arr = np.array(np.sqrt(np.sum((np.matlib.repmat(T, np.shape(wine_set)[0], 1) - wine_set) ** 2, 1)))
    # lamb_arr[::-1].sort()
    # wine_rdpm_assignment = RDPM(wine_set, lamb_arr[2], wine_const, 20000, 0.1, 1)[0]
    ...
    # lamb_arr[::-1].sort()
    # breast_cancer_rdpm_assignment = RDPM(breast_cancer_set, lamb_arr[1], breast_cancer_const, 20000, 0.1, 1)[0]
    # T = np.mean(glass_set, 0)
    # lamb_arr = np.array(np.sqrt(np.sum((np.matlib.repmat(T, np.shape(glass_set)[0], 1) - glass_set) ** 2, 1)))
    # lamb_arr[::-1].sort()
    # glass_rdpm_assignment = RDPM(glass_set, lamb_arr[5], glass_const, 20000, 0.1, 1)[0]
    # T = np.mean(digits_set, 0)
    # lamb_arr = np.array(np.sqrt(np.sum((np.matlib.repmat(T, np.shape(digits_set)[0], 1) - digits_set) ** 2, 1)))
    # lamb_arr[::-1].sort()
    # digits_rdpm_assignment = RDPM(digits_set, lamb_arr[9], digits_const, 20000, 0.1, 1)[0]
    #
